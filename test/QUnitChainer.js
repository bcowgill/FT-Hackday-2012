...
var QUnitChainer = {
   ...
   Lists: {},                // named, ordered, unique lists of test plans to run
   ...
   fetchItem: function (key, bJSON) {
      var value = undefined, rStorage = this.getStorage();
      if (rStorage) {
         try {
   ...
   clearStorage: function () {
      ...
      this.clearList('failures');
      this.clearList('runlist');
      this.storeLists();
      return this;
   },
   ...
   /*
    * QUnitChainer.clearList(key)
    *
    * Clear the list named by 'key'
    * If key is omitted then all lists are cleared from storage
    */
   clearList: function (key) {
     if (key) {
        this.Lists[key] = [];
     } else {
         this.removeItem(this.lskey);
         this.Lists = this.getList();
     }
      return this;
   },
   ...
   /*
    * QUnitChainer.addToList(key, item)
    *
    * Add an item to the list named by 'key' only if it's not presently in the list.
    */
   addToList: function (key, item) {
      var idx, bFound = false, List = this.Lists[key];
      if (!List) {
           this.Lists[key] = [item];
      } else {
   ...
   /*
    * QUnitChainer.getList(key)
    *
    * Retrieve the list named by 'key' or retrieve all lists from storage if key omitted
    */
   getList: function (key) {
     var rLists;
      if (key) {
         ...
         rLists = this.fetchItem(this.lskey, 'JSON');
         if (!rLists) {
            rLists= { _runorder: "_runorder", runlist: [], failures: [] };
         }
         return rLists;
      }
   },
   ...
   /*
    * QUnitChainer.storeLists(rLists)
    *
    * Save the named lists to storage
    *
    * rLists - optional object holding named lists to store
    *    Default is QUnitChainer.Lists
    */
   storeLists: function (rLists) {
      rLists = rLists || this.Lists;
      this.storeItem(this.lskey, rLists, 'JSON');
      return this;
   },

   /*
    * QUnitChainer.nextFromList(key, item)
    *
    * Retrieve the next item in the list named by 'key' after the item 'item'.
    * returns undefined if the item is not in the list or there is no item after it.
    */
   nextFromList: function (key, item) {
      var idx, nextItem = undefined, bFound = false, List = this.Lists[key];
      if (List) {
         for (idx = 0; idx < List.length; ++idx) {
            if (bFound) {
               nextItem = List[idx];
               break;
            } else if (item === List[idx]) {
               bFound = true;
            }
         }
      }
   ...
   /*
    * QUnitChainer.removeFromList(key, item)
    *
    * Remove an item from the list named by 'key'.
    * Does nothing if the list or item doesn't exist.
    */
   removeFromList: function (key, item) {
      var idx, bFound = false, List = this.Lists[key];
      if (List) {
         for (idx = 0; idx < List.length; ++idx) {
            if (item === List[idx]) {
   ...
   /*
    * QUnitChainer.shouldSaveResults()
    *
    * Determine if we should save the current test results to storage.
    * If the URL is a full test plan URL then we save results. If it indicates that it is a single test run
    * then we don't save results.
    *
    */
   shouldSaveResults: function () {
      return (this.Tests.plan.match(/(\?|%3F)(filter|spec)=/i)) ? false : true;
   },

   /*
    * QUnitChainer.getNextTestPlan()
    * Determine the next test plan to run after the current one.
    * This will use the nextTestPlan property if the Lists._runorder is set to '_runorder'
    * If the Lists._runorder is set to an existing list within Lists then the next test plan
    * will be taken from that list.
    */
   getNextTestPlan: function () {
      var nextTestPlan = this.nextTestPlan;
     if (this.Lists._runorder && this.Lists._runorder !== '_runorder' && this.Lists[this.Lists._runorder]) {
         nextTestPlan = this.nextFromList(this.Lists._runorder, this.cleanURL());
     }
      return nextTestPlan;
   },
    ...
    * add the test plan url to the runlist list of test plans
    *
   ...
   begin: function () {
      ...
      self.Tests.plan      = self.docLocation();
      if (self.shouldSaveResults()) {
          self.addToList('runlist', self.cleanURL());
          self.storeLists();
      }
    ...
    */
      ...
      var userAgent, testPlan, nextTestPlan, rTestStorage, rJQ, self = QUnitChainer;
      ...
      if (self.shouldSaveResults()) {
         ...
         if (self.testIsFailure(self.Tests)) {
             self.addToList('failures', self.cleanURL());
         } else {
             self.removeFromList('failures', self.cleanURL());
         }
         self.storeLists();

         // Chain to next test plan if flag is set and next test plan is defined
         nextTestPlan = self.getNextTestPlan();
         if (self.getProperty('bFollowChain') && nextTestPlan) {
            self.logEvent({ 'in': 'QUC.done()', 'nextTestPlan': nextTestPlan });
            if (self.bPause) {
               self.myAlert("Tests finished, chaining to " + nextTestPlan + "\nfrom " + self.getLocation());
            }
            self.maybeAlertStorage('QUC - QUnit.done() - chain');
            self.setLocation(nextTestPlan);
   ...
   injectControlPage: function (jqInjectAt) {
      ...
      if (jQuery('#qunit-header').length === 0) {
         html = [
            ...
            '<br><div id="groupRunList" style="display: none;"><select id="runListName" name="runListName" size="1">',
            '<option value="newRunList" selected>- new run list -</option>',
            '<optgroup label="internally defined">',
            '<option value="runlist" >runlist</option>',
            '<option value="failures">failures</option>',
            '</optgroup>',
            '<optgroup label="user defined">',
            '</optgroup>',
            '</select>',
            '<input type="button" id="newRunList" name="newRunList" value="new list">',
            '<input type="button" id="saveRunList" name="saveRunList" value="save list">',
            '<input type="button" id="deleteRunList" name="deleteRunList" value="delete list">',
            '<br><textarea id="runListContents" name="runListContents" rows="40" cols="80" placeholder="first line is run list name. Subsequent lines hold each test plan URL to run."></textarea>',
            '</div></div>',
            '</form>',
   ...
   /*
    * QUnitChainer.updateRunListFields(runList)
    *
    * Update the form fields on the control page which relate to the run list.
    */
   updateRunListFields: function (runList) {
      runList = runList || this.Lists._runorder || '_runorder';
      if (runList === '_runorder') {
         jQuery('#runListName')[0].selectedIndex = 0;
         jQuery('#runListContents').html('');
      } else {
         jQuery('#runListName').val(runList);
         jQuery('#runListContents').html('[' + runList + ']\n' + this.getList(runList).join("\n"));
      }
   },
    ...
    *
    * Show the Properties, History, Lists and Test Summary Results which are
    * saved in storage in a JSON dump for debugging.
    * They will be displayed within the DIV with ID qunitchainer-dump
    */
   dumpStorage: function () {
      var rTestResults, raHistory, rLists, dump = '';
      if (this.getProperty('bDumpStorage')) {
         ...
         dump = [
            ...
            '<b>' + this.storage + '[' + this.lskey + ']</b>',
            '<pre>' + this.stringifyObj(rLists) + '</pre>',
         ...
         if (rTestResults.hasOwnProperty(userAgent)) {
            ...
            for (planURL in rTestResults[userAgent]) {
               if (rTestResults[userAgent].hasOwnProperty(planURL)) {
                  ...
                  moduleStatus = this.testIsFailure(rTestPlan) ? 'fail' : 'pass';
   ...
   /*
    * QUnitChainer.testIsFailure(rTests)
    *
    * Checks if a test run is a failure. A failure happens if no tests were run
    * or if any tests failed.
    */
   testIsFailure: function (rTests) {
     return (rTests.failed || (rTests.total === 0));
   },
